...
Module Store
    ...
    Public lst0 As New List(Of String)
    Public lst1 As New List(Of String)

    Public Resources As String() = {Application.StartupPath & "\"}
    Public Function Encoding() As System.Text.Encoding
        Dim i As System.Text.Encoding = System.Text.Encoding.UTF8
        Return i
    End Function
    Public Function SplitByte(ByVal b As Byte(), ByVal s As String) As Array
        Dim L As New List(Of Byte())
        Dim c As Integer = -1
        c = Array.IndexOf(Of Byte)(b, CByte(Strings.Asc(s.Chars(0))))
        Do While True
            If Not ((c > -1) And ((c + s.Length) <= b.Length)) Then
                Exit Do
            End If
            Dim buff As Byte() = New Byte(((s.Length - 1) + 1) - 1) {}
            Dim int0 As Integer = 0
            Dim int1 As Integer = ((c + s.Length) - 1)
            Dim int3 As Integer = c
            Do While (int3 <= int1)
                buff(int0) = b(int3)
                int0 += 1
                int3 += 1
            Loop
            If (Encoding().GetString(buff) = s) Then
                Dim ms As New IO.MemoryStream
                ms.Write(b, 0, c)
                L.Add(ms.ToArray)
                ms.Dispose()
                ms = New IO.MemoryStream
                ms.Write(b, (c + s.Length), (b.Length - (c + s.Length)))
                L.Add(ms.ToArray)
                ms.Dispose()
                Exit Do
            End If
            c = Array.IndexOf(Of Byte)(b, CByte(Strings.Asc(s.Chars(0))), (c + 1))
        Loop
        If (L.Count = 0) Then
            L.Add(b)
        End If
